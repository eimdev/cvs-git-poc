 ...
 */
public class NEFTOutwardReturnedReportAction 
extends AbstractReportAction {

    private static Category logger = Category.getInstance(NEFTOutwardReturnedReportAction.class);

    @Override
    protected String getActionName(String mode) {

        String actionName = "insta.vijb.report.owReturnedReport";
    ...
    throws Exception {
        ...
        if (mode!=null && mode.equalsIgnoreCase("viewreport")) {
            ...
            try {
                ...
                reportBean.generateNEFTOwReturnedReport(request);
            ...
            try {
                ...
                String fileName = "Outward Returned Report ";
                
                if (reportBean.getReportDTOs().size() > 0) {
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename="+fileName+".xls");
                    reportBean.generateNEFTOwReturnedExportToExcel(response.getOutputStream());
                } else {
