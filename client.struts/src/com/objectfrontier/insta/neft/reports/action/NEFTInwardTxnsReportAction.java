/*
 * @(#)NEFTInwardTxnsReportAction.java
 *
 ...
 * @author mohanadevis
 * @date   Feb 24, 2009
 * @since  insta.reports; Feb 24, 2009
 */
public class NEFTInwardTxnsReportAction 
extends AbstractReportAction {
    
    private static Category logger = Category.getInstance(NEFTInwardTxnsReportAction.class.getName());
    
    @Override
    protected String getActionName(String mode) {

        String actionName = "insta.neft.report.inwardtxnsreport";
    ...
    throws Exception {
        ...
        if (mode!=null && mode.equalsIgnoreCase("viewreport")) {
            ...
            try {
                ...
                reportBean.generateNEFTInwTxnsReport(request);
            ...
            try {
                ...
                String fileName = "Inward Transactions - Detail Report";
                if (reportBean.inwardTxns.size() > 0) {
                    
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename="+fileName+".xls");
                    reportBean.inwTxnsDetailExportToExcel(response.getOutputStream());
                } else {
