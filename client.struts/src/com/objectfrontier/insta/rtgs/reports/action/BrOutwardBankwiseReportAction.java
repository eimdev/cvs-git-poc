 ...
 */
public class BrOutwardBankwiseReportAction 
extends AbstractReportAction {

    private static Category logger = Category.getInstance(BrOutwardBankwiseReportAction
                                                          .class);

    @Override
    protected String getActionName(String mode) {

        String actionName = "insta.vijb.report.brOutwardBankWise";
    ...
    throws Exception {
        ...
        if (mode!=null && mode.equalsIgnoreCase("viewreport")) {
            ...
            try {
                ...
                reportBean.generateOutwardBankwiseReport(request);
            ...
            try {
                ...
                String fileName = "Outward - Bank Wise Report ";
                
                if (reportBean.getReportMap().size() > 0) {
                    reportBean.setReport("outward");
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename="+fileName+".xls");
                    reportBean.generateBankwiseExportToExcel(response.getOutputStream());
                } else {
