 ...
 */
public class BrIndividualReportAction 
extends AbstractReportAction {

    private static Category logger = Category.getInstance(BrIndividualReportAction.class);

    @Override
    protected String getActionName(String mode) {

        String actionName = "insta.vijb.report.brIndReport";
    ...
    throws Exception {
        ...
        if (mode!=null && mode.equalsIgnoreCase("viewreport")) {
            ...
            try {
                ...
                reportBean.generateBrIndividualReport(request);
            ...
            try {
                ...
                String fileName = "Branchwise Individual Report ";
                
                if (reportBean.getReportMap().size() > 0) {
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename="+fileName+".xls");
                    reportBean.generateBrIndividualExportToExcel(response.getOutputStream());
                } else {
